...
class TestRegister:
    ...
    async def test_register_duplicate_username(self, client: httpx.AsyncClient) -> None:
        await register_user(client, username="taken")
        resp = await client.post("/v1/auth/register", json={
            "username": "taken",
            "password": "SecurePass123!",
        })
    ...
    async def test_register_empty_username(self, client: httpx.AsyncClient) -> None:
        resp = await client.post("/v1/auth/register", json={
            "username": "",
            "password": "SecurePass123!",
        })
...
class TestLogin:
    async def test_login_success(self, client: httpx.AsyncClient) -> None:
        await register_user(client, username="login-user", password="MyPassword123!")
    ...
    async def test_login_wrong_password(self, client: httpx.AsyncClient) -> None:
        await register_user(client, username="wrong-pw", password="CorrectPass123!")
        resp = await client.post("/v1/auth/login", json={
            "username": "wrong-pw",
    ...
    async def test_login_nonexistent_username(self, client: httpx.AsyncClient) -> None:
        resp = await client.post("/v1/auth/login", json={
            "username": "nobody",
...
class TestMe:
    async def test_me_authenticated(self, client: httpx.AsyncClient) -> None:
        auth = await register_user(client, username="me-test")
