...
@router.patch(
    ...
    description=(
        ...
        "compatibility.\n\n"
        "**Cannot update `content` or `content_format` here.** Entry content "
        "is the entry's git repository, which can hold many files (a "
        "markdown entry might be just `.phiacta/content.md`; a LaTeX entry "
        "can span `main.tex` + `references.bib` + `figures/...` + "
        "`sections/...`). Since content is multi-file in general, it "
        "cannot be sent as a single `content` field — write files "
        "directly:\n"
        "- **Owner**: `PUT /v1/entries/{id}/files/{path}` (MCP tool: "
        "`put_entry_file`). The main content file is at "
        "`.phiacta/content.<ext>` matching `content_format`.\n"
        "- **Non-owner**: `POST /v1/entries/{id}/edits` (MCP tool: "
        "`create_edit_proposal`) — opens a branch + PR; the owner reviews "
        "and merges.\n\n"
