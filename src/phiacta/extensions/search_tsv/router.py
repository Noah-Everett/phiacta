...
"""search_tsv extension API router.

Endpoints:
- GET /version       Active version metadata (public)
- GET /{entry_id}    Raw tsvector for an entry (visibility-checked)

IMPORTANT: /version is registered BEFORE /{entry_id} to avoid route
collision — FastAPI matches routes in registration order.

The router is mounted at /v1/extensions/search_tsv/ by the plugin framework.
"""
...
from phiacta.extensions.search_tsv.schemas import (
    SearchTsvResponse,
...
async def get_version(
    ...
    """Return the active version metadata for the search_tsv extension.
...
async def get_tsvector(
    ...
    Checks visibility — private entries return 403 for non-owners.
    """
    # Visibility check: private entries are restricted
